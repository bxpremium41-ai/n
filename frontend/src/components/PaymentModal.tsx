...
const BACKEND_URL = ENV_BACKEND_URL 
    ? `${ENV_BACKEND_URL}/create-payment-intent`
...
export const PaymentModal: React.FC<PaymentModalProps> = ({ isOpen, onClose }) => {
  ...
  const hasInitializedStripe = useRef(false);
  ...
  const resetModal = () => {
    ...
    hasInitializedStripe.current = false;
  ...
  const initializeStripe = async (retry = 0) => {
    try {
        if (!window.Stripe) {
            if (retry < 3) setTimeout(() => initializeStripe(retry + 1), 500);
            return;
        }
                         ...
                         <div className="w-full space-y-2">
                             {COURSES.map((course, idx) => {
                                 ...
                                 return (
                                     <div 
                                        key={course.id} 
                                        className={`flex items-center justify-between p-3 rounded-xl border transition-all duration-300 ease-out transform ${
                                            isAdded 
                ...
                {viewState === 'CONNECTION_ERROR' && (
                    <div className="h-full flex flex-col items-center justify-center text-center space-y-4">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-500 mt-2">Cannot reach {BACKEND_URL}</p>
                        </div>
                        <button onClick={() => { setViewState('LOADING'); setTimeout(initializeStripe, 500); }} className="px-6 py-3 bg-gray-900 text-white rounded-xl font-bold text-sm flex items-center gap-2">
            ...
            {(viewState === 'FORM' || viewState === 'PROCESSING') && paymentMethod === 'card' && (
                <div className="p-5 md:p-6 border-t border-gray-100 bg-white shrink-0 z-20 pb-6 md:pb-6">
                    <button
                        ...
                        disabled={viewState === 'PROCESSING' || !isStripeLoaded}
